...
from userbot import MONGO_URI
import pymongo

# ================= CONSTANT =================
DEFAULTUSER = Config.ALIVE_NAME if Config.ALIVE_NAME else uname().node
URI = str(MONGO_URI) if MONGO_URI else "No var set yet."
# ============================================
...
#Some variables
client = pymongo.MongoClient(f"{URI}")
db = client.test
current_msgs = {}
...
@borg.on(admin_cmd(pattern="listDB"))
...
async def chat_bot(event):
	...
	print(reply)
	await event.edit(f"**{DEFAULTUSER}:\n"+str(reply)+"**")

@borg.on(admin_cmd(pattern="autochat"))
async def chat_bot(event):
	if event.fwd_from:
		return  
	if MONGO_URI is None:
...
@borg.on(admin_cmd(pattern="stopchat"))
async def chat_bot(event):
	if event.fwd_from:
		return  
	if MONGO_URI is None:
...
@borg.on(admin_cmd(pattern="learnstart ?(.*)"))
...
@borg.on(admin_cmd(pattern="learnstop ?(.*)"))
