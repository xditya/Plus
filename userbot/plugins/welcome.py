...
from userbot.plugins.sql_helper.global_variables_sql import DEFAULTUSER
...
async def del_welcome(event):
    ...
    if rm_welcome_setting(event.chat_id) is True:
        await event.edit(f"`{DEFAULTUSER}`: **Welcome note deleted for this chat.**")
    else:
        await event.edit(f"`{DEFAULTUSER}`: ** I Didnt have any welcome messages here **")
...
async def show_welcome(event):
    ...
    cws = get_current_welcome_settings(event.chat_id)
    if not cws:
        await event.edit(f"`{DEFAULTUSER}`: **No welcome message saved here.**")
        return
    elif cws and cws.f_mesg_id:
        msg_o = await event.client.get_messages(entity=BOTLOG_CHATID,
                                                ids=int(cws.f_mesg_id))
        await event.edit(
            f"`{DEFAULTUSER}`: **I am currently welcoming new users with this welcome note.**")
...
async def save_welcome(event):
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            ...
            await event.edit(
                f"`{DEFAULTUSER}`: **Saving media as part of the welcome note requires the BOTLOG_CHATID to be set.**"
            )
            return
    ...
    if add_welcome_setting(event.chat_id, 0, string, msg_id) is True:
        ...
        await event.edit(f"`{DEFAULTUSER}`: **Ops, old welcome message found, i deleted it Sucessfully now you can save new welcome message!**")
